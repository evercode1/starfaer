<form class="mt-20"
      role="form"
      method="POST"
      action="{{ url('/galaxy-type/'. $galaxyType->id) }}"
...
<!-- name input -->
    ...
    <!-- is_active select -->
        ...
        <div class="{{ $errors->has('is_active') ? ' has-error' : '' }}">
            ...
            <select id="is_active" name="is_active">
                ...
                <option value="{{ $galaxyType->is_active }}">{{ $galaxyType->is_active == 1 ? 'Yes' : 'No' }}</option>
    ...
    <div class="{{ $errors->has('is_featured') ? 'has-error' : '' }}">
        ...
        <select id="is_featured" name="is_featured">
            <option value="{{ $galaxyType->is_featured }}">{{ $galaxyType->is_featured == 1 ? 'Yes' : 'No' }}</option>

            @if($galaxyType->is_featured == 1)
    ...
    <div class="{{ $errors->has('description') ? ' has-error' : '' }}">
        ...
        <textarea id="description" name="body">{!! $galaxyType->description !!}</textarea>
